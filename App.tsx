...
function App() {
  ...
  useEffect(() => {
    ...
    const initLocalMode = () => {
      ...
      setUserId(localId);
      loadData(localId).then(d => { setData(normalizeData(d)); setLoading(false); });
    };
    if (auth) {
      const unsubscribeAuth = onAuthStateChanged(auth, async (user) => {
        if (user) {
          ...
          setLoading(false);
          unsubscribeData = subscribeToData(user.uid, (newData) => { setData(normalizeData(newData)); });
        } else {
          try { await signInUser(); } catch (e) { initLocalMode(); }
        }
      });
  ...
  useEffect(() => { if (userId && !loading) saveData(userId, data); }, [data, userId, loading]);
  ...
  const createNewSection = (type: 'income' | 'expense') => {
    ...
    // Todas as saídas agora usam estrutura de parcelamento por padrão
  ...
  const updateSection = (updatedSection: CustomSection) => {
    handleUpdate({ ...data, customSections: data.customSections.map(s => s.id === updatedSection.id ? updatedSection : s) });
  };
