...
import { RefreshCw, Plus, LogOut, User as UserIcon, CloudCheck, Cloud } from 'lucide-react';
...
function App() {
  ...
  useEffect(() => {
    ...
    const normalizeData = (d: FinancialData) => {
      ...
      if (!d.incomeModulesOrder) d.incomeModulesOrder = ['incomes'];
  ...
  // Efeito para salvar dados com indicador de sincronia
  useEffect(() => {
    if (userId && !loading) {
      setIsSyncing(true);
      const timer = setTimeout(async () => {
        ...
        setIsSyncing(false);
      }, 500); // Debounce para não sobrecarregar a rede
      return () => clearTimeout(timer);
    }
  }, [data, userId, loading]);

  const handleUpdate = (newData: FinancialData) => setData(newData);

  const createNewSection = (type: 'income' | 'expense') => {
    const title = prompt(`Nome da nova sessão de ${type === 'income' ? 'Entrada' : 'Saída'}?`);
    if (!title) return;

    const structure = type === 'expense' ? 'installment' : 'standard';
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-neon-dark flex flex-col items-center justify-center text-neon-blue gap-4">
        ...
        <p className="text-[10px] font-bold uppercase tracking-[0.3em] animate-pulse">Sincronizando Dados...</p>
      </div>
    );
  }
    ...
    <div className="min-h-screen text-slate-200 pb-20 selection:bg-neon-pink selection:text-white">
      <nav className="border-b border-white/5 bg-neon-surface/80 backdrop-blur-md sticky top-0 z-50 shadow-[0_4px_30px_rgba(0,0,0,0.5)]">
        <div className="max-w-7xl mx-auto px-4 py-3 flex flex-col md:flex-row items-center justify-between gap-4">
          ...
          <button 
            onClick={logoutUser}
            className="flex items-center gap-2 text-[10px] font-bold text-slate-400 hover:text-neon-red transition-all uppercase tracking-widest px-4 py-2 hover:bg-neon-red/10 rounded-lg border border-transparent hover:border-neon-red/30"
          >
      ...
      <footer className="mt-20 py-8 text-center text-slate-600 text-[10px] font-bold tracking-widest uppercase">
        FINANCIAL CONTROLLER • {new Date().getFullYear()} • Cloud Sync Active
      </footer>
    </div>
  );
}
